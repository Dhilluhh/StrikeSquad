...
const CharacterCustomization = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/api/users/${user.id}/stats`, { credentials: 'include' })
    ...
    const fetchConfig = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/api/users/${user.id}/character`, { credentials: 'include' })
        const data = await response.json()
        if (data && !data.error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/users/${user.id}/character`, {
        method: 'POST',
