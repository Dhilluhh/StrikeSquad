...
// Middleware
app.use(cors({
  origin: [process.env.CLIENT_URL, 'http://localhost:5173', 'http://localhost:3000'].filter(Boolean), // Allow Config, Vite Default, and Custom Port
  credentials: true
}))
  ...
  try {
    ...
    res.json({
      ...
      healthHistory: historyRows.map(row => ({
        ...
        focusScore: Math.round(row.focus_score || 0),
        sessionsCompleted: row.total_sessions || 0,
        sessionsFailed: row.failed_sessions || 0,
      })).reverse(),
    })
  } catch (error) {
